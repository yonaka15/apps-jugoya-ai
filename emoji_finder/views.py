...
def index(request):
    ...
    メインページを表示する
    """
    return render(request, "emoji_finder/index.html")


def query(request):
    """
    絵文字検索APIにクエリを送信し、結果をJSONで返す
    GETメソッドに対応
    ...
    try:
        api_url = f"{os.getenv('API_HOST')}{os.getenv('EMOJI_FINDER_ENDPOINT')}"
        headers = {
        ...
        try:
            ...
            # データはすでに配列形式なのでそのまま使用
            response_data["emojis"] = data
