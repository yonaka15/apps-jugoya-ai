...
async function initializeWebRTC() {
  try {
    ...
    if (userInstructions) {
      ...
      addDebugLog(
        `Saving user instructions for later use: "${currentInstructions}"`
      );
    }
    ...
    // セッショントークンを取得
    const token = await getSessionToken(selectedModel, currentVoice);
    ...
    async function autoUpdateSessionAfterCreation() {
      ...
      try {
        ...
        if (selectedInstructions) {
          addDebugLog(
            `Auto-updating with instructions: "${selectedInstructions}"`
          );
        } else {
          addDebugLog("Auto-updating without custom instructions");
        }
        addDebugLog(
          `Auto-updating with modalities: ${selectedModalities.join(", ")}`
        );
      ...
      try {
        ...
        if (
          ...
          if (typeof textContent === "string") {
            // テキストコンテンツが空文字でも処理する
            addDebugLog(
              `Received text delta: [${textContent.length} chars] (type: ${realtimeEvent.type})`
            );
            ...
            addDebugLog(
              `Invalid response.text/delta event: ${JSON.stringify(
                realtimeEvent
              )}`
            );
          }
        ...
        // response.text.done イベントの処理を追加
          ...
          addDebugLog(
            `Received text done event with text: ${realtimeEvent.text?.substring(
              0,
              50
            )}...`
          );
          ...
          if (typeof realtimeEvent.text === "string") {
            ...
            addDebugLog(
              `Final text content: ${displayText.substring(0, 50)}...`
            );
          }
        ...
        // セッション更新イベントの処理
          ...
          if (isUpdatingSession) {
            ...
            if (
              ...
              realtimeEvent.session.instructions !== undefined
            ) {
        ...
        // session.createdイベントの処理（変更あり - 自動セッション更新を追加）
          ...
          if (
            ...
            elements.instructionsInput.value = currentInstructions;
            addDebugLog(
              `Using default instructions from server: "${currentInstructions}"`
            );
          } else if (userInstructions) {
            // ユーザーが入力済みの場合はその値を維持
            currentInstructions = userInstructions;
            addDebugLog(
              `Keeping user-provided instructions: "${currentInstructions}"`
            );
          }
        ...
        // 変更: トランスクリプトのデルタ更新と完了イベントの処理を追加
          ...
          if (typeof realtimeEvent.delta === "string") {
            addDebugLog(
              `Received transcript delta: [${realtimeEvent.delta.length} chars]`
            );
            updateTranscriptText(realtimeEvent.delta);
          } else {
            addDebugLog(
              `Invalid transcript delta: ${JSON.stringify(realtimeEvent)}`
            );
          }
        }
        // 完了したトランスクリプトの処理 - 従来のサポートを維持しつつ、デルタ処理も追加
          ...
          if (typeof realtimeEvent.transcript === "string") {
            ...
            addDebugLog(
              `Invalid transcript done event: ${JSON.stringify(realtimeEvent)}`
            );
          }
        }
...
async function updateSession() {
  ...
  try {
    ...
    if (selectedModalities.length === 0) {
      ...
      showError(
        "少なくとも1つの応答形式（テキストまたは音声）を選択してください"
      );
      isUpdatingSession = false;
      updateStatus("Ready");
      elements.updateStatus.classList.add("hidden");
    ...
    addDebugLog(
      `Updating session with instructions: "${selectedInstructions}"`
    );
    addDebugLog(
      `Updating session with modalities: ${selectedModalities.join(", ")}`
    );
    ...
    setTimeout(() => {
      if (isUpdatingSession) {
        ...
        elements.updateSessionButton.disabled = false;
        showError(
          "Session update timed out. Settings may not be changeable if audio has already been generated."
        );
...
async function sendMessage() {
  ...
  try {
    ...
    try {
      ...
      addDebugLog(
        `Requesting response with modalities: ${selectedModalities.join(", ")}`
      );
